...
export async function paymentRoutes(app: FastifyInstance) {
  ...
  app.post('/api/cakto/webhook', async (request, reply) => {
    ...
    try {
      ...
      // Server-side tracking: enviar evento Purchase para Stape/GA4
      if (order.ga_client_id) {
        try {
          await sendPurchaseToStape({
            gaClientId: order.ga_client_id,
            transactionId: order.id,
            value: amount_reais > 0 ? amount_reais : (order.amount_cents || 0) / 100,
            currency: 'BRL',
            customerEmail: order.customer_email,
          });
        } catch (trackingError) {
          console.warn('[Cakto Webhook] Erro no tracking server-side (não crítico):', trackingError);
        }
      }
